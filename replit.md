...
- **YT-DLP PERMISSION FIX**: Fixed yt-dlp permission error on VPS by adding `--no-cookies` flag and `no_cookies: True` option to all yt-dlp configurations. This prevents yt-dlp from trying to save cookies to read-only directories, resolving "PermissionError: [Errno 13]" errors on Hostinger VPS. Created deployment/fix-ytdlp.sh script to update yt-dlp and test on VPS.
